...
type Task struct {
	...
	AssigneeID  uint      `json:"assignee_id"`                                      // 负责人ID
	ProjectID   uint      `json:"project_id" gorm:"index:idx_tasks_project_status"`  // 项目ID（复合索引）
	Status      TaskStatus `json:"status" gorm:"type:varchar(50);not null;default:'todo';index:idx_tasks_project_status"` // 状态（复合索引）
